''' This code is copied from https://github.com/sniklaus/pytorch-hed/ and remove the sigmoid'''
...
class HED(torch.nn.Module):
    def __init__(self, classes, pretrained=False):
        ...
        self.netScoreOne = torch.nn.Conv2d(in_channels=64, out_channels=classes, kernel_size=1, stride=1, padding=0)
        self.netScoreTwo = torch.nn.Conv2d(in_channels=128, out_channels=classes, kernel_size=1, stride=1, padding=0)
        self.netScoreThr = torch.nn.Conv2d(in_channels=256, out_channels=classes, kernel_size=1, stride=1, padding=0)
